...
// Get system settings for academic year and term (key-value structure)
...
$term = $_GET['term'] ?? ($system_settings['current_term'] ?? 'First Term');
...
foreach ($students as $student) {
    ...
    foreach ($subjects as $subject) {
        $score_query = "SELECT total_score as score FROM scores 
                      WHERE student_id = ? AND subject_id = ?
                      AND academic_year = ? AND term = ?";
        $score_stmt = $db->prepare($score_query);
        $score_stmt->execute([$student['id'], $subject['id'], 
                            $academic_year, $term]);
...
foreach ($subjects as $subject) {
    ...
    // Check if core subject by name
    $core_subjects = ['English Language', 'Mathematics', 'Integrated Science', 'Social Studies'];
    $is_core = in_array($subject['subject_name'], $core_subjects);
...
foreach ($students_data as &$student_data) {
    ...
    // Only calculate aggregate if student has all required subjects
    if (count($core_grades) < 4 || count($elective_grades) < 2) {
        $student_data['aggregate'] = null; // Cannot calculate - incomplete data
        continue;
    }
...
// Filter out students with null aggregates and sort by aggregate
$students_with_aggregates = array_filter($students_data, function($s) {
    return $s['aggregate'] !== null;
});
usort($students_with_aggregates, function($a, $b) {
    return $a['aggregate'] <=> $b['aggregate'];
});

// Flatten student data for easier access in PDF
$flattened_students = [];
foreach ($students_with_aggregates as $student_data) {
    $flattened_students[] = [
...
$top_performers = array_slice($students_with_aggregates, 0, 5);
...
try {
    ...
    <body>
        <div class="header">
            ...
            <h1>TERMLY EXAMINATION DETAILED ANALYSIS</h1>
            <p><strong>Class:</strong> <?php echo htmlspecialchars($class_info['class_name']); ?> | 
               <strong>Academic Year:</strong> <?php echo htmlspecialchars($system_settings['current_academic_year'] ?? $academic_year); ?> | 
               <strong>Term:</strong> <?php echo htmlspecialchars($system_settings['current_term'] ?? $term); ?></p>
        ...
        <table>
            ...
            foreach ($aggregate_ranges as $range => $category):
                ...
                $row_class = '';
                if ($category == 'Excellent') $row_class = 'grade-1-3';
                elseif ($category == 'Good') $row_class = 'grade-4-6';
                elseif ($category == 'Satisfactory') $row_class = 'grade-7-8';
        ...
        <div class="grade-guide">
            <strong>Ghana Education Service Grading System:</strong> 
        ...
        <?php if (empty($class_stats['subjects'])): ?>
        <p style="margin: 10px 0; padding: 10px; background-color: #fff3cd; border: 1px solid #ffc107; color: #856404; font-size: 9pt;">
            <strong>Note:</strong> No scores have been recorded for this class yet. Please ensure scores are entered for the selected academic year and term.
        </p>
        <?php endif; ?>
        ...
        <table>
            ...
            foreach ($flattened_students as $student):
                ...
                // Handle ties - same aggregate = same rank
                if ($previousAggregate !== null && $aggregate > $previousAggregate) {
                    $rank = $currentRank;
                }
                $previousAggregate = $aggregate;
                $agg_category = '';
                if ($aggregate >= 6 && $aggregate <= 12) $agg_category = 'Excellent';
                elseif ($aggregate <= 24) $agg_category = 'Good';
                elseif ($aggregate <= 36) $agg_category = 'Satisfactory';
                else $agg_category = 'Fail';
                
                $row_class = '';
                if ($agg_category == 'Excellent') $row_class = 'grade-1-3';
                elseif ($agg_category == 'Good') $row_class = 'grade-4-6';
                elseif ($agg_category == 'Satisfactory') $row_class = 'grade-7-8';
                else $row_class = 'grade-9';
            ?>
            <tr class="<?php echo $row_class; ?>">
        ...
        <table>
            ...
            foreach ($flattened_students as $student):
                ...
                $gender_char = strtoupper(substr($student['gender'] ?? '', 0, 1));
                if ($gender_char == 'M') {
                    $gender_display = 'Male';
                } elseif ($gender_char == 'F') {
                    $gender_display = 'Female';
                } else {
                    $gender_display = $student['gender'] ?? 'Unknown';
                }
            ?>
            <tr class="<?php echo $row_class; ?>">
        ...
        <div class="footer">
            ...
            <p><strong>Termly Examination - Detailed Statistical Analysis</strong></p>
        </div>
    </body>
    </html>
    <?php
    ...
    $mpdf->SetTitle('Termly Exam Analysis - ' . $class_info['class_name']);
    ...
    // Output PDF
    $filename = 'Termly_Analysis_' . $class_info['class_name'] . '_' . date('Y-m-d') . '.pdf';
